    ...
    def __init__( self, meshFile, skeletonFile, correspondanceFile ):
        ...
        with open( skeletonFile ) as file:
            for idx, line in enumerate(file):
                ...
                self.branches.append( SkeletonBranch( np.reshape( np.array( contents[1:]).astype(np.float32), ((len(contents) - 1) // 3, 3) ) ))
...
def calculateCovMatrix( size, branchNumber, std ):    
    mat = np.zeros( (size,size) ) + np.eye(size,size) * 1e-20 # pytorch me pide que sea positiva definida... le pongo valor muy chico
    mat[branchNumber, branchNumber] = 1e-20 # std
    return mat.tolist()
