...
def generate_pc_full( config ):
    gen = Sampler( 3, checkpoint=config['model_path'], device=config['device'], threshold=0.3, w0=config['weight0'] )
...
def generate_pc( config ):
    if config['json'] == '':
        ...
        with open(config['json']) as jsonFile:
            ...
            gen = Sampler( len(skel['joints'][0]['mean']) + 3, checkpoint=config['model_path'], device=config['device'], threshold=0.3, w0=config['weight0'] )
